...
class ContaCorrente:
    ...
    def __init__(self, cliente, agencia, numero):
        ...
        self.__agencia = 0
        self.__numero = 0
        self.saques_não_permitidos = 0
        self.transferencias_não_permitidas = 0
    ...
    def transferir(self, valor, favorecido):
        if valor < 0:
            self.transferencias_não_permitidas += 1
        ...
        try:
            ...
            self.transferencias_não_permitidas += 1
            E.args = ()
            raise OperacaoFinanceiraError("Operação não finalizada", E) from E
        favorecido.depositar(valor)
    
    def sacar(self, valor):
        if valor < 0:
            self.saques_não_permitidos += 1
            raise ValueError("O valor a ser sacado não pode ser menor que zero")
        if self.saldo < valor:
            self.saques_não_permitidos += 1
